...
namespace MiniORM
{
    public abstract class DbContext
    {
        ...
        private void MapNavigationProperties<TEntity>(DbSet<TEntity> dbSet) where TEntity : class, new()
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                foreach (var entity in dbSet)
                {
                    ...
                    var navigationPropertyValue = ((IEnumerable<object>) navigationDbSet).First(cnp =>
                        navigationPrimaryKey.GetValue(cnp).Equals(foreignKeyValue));
        ...
        {
            foreach (var dbSetPropety in dbSetProperties)
            {
                var dbSetType = dbSetPropety.Key;

                var mapRelationGeneric = typeof(DbContext)
                    .GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic)
                    .MakeGenericMethod(dbSetType);

                var dbSet = dbSetPropety.Value.GetValue(this);
        ...
        private bool IsObjectValid(object entity)
        {
            ...
            var validationResult = Validator.TryValidateObject(entity, validationContext, validationErrors,
